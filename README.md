...
## Maximizing return via time series modeling 

# Scenario:
### As an analyst for a real estate investment company I am tasked with finding US zip codes to invest in using time series modeling to predict future prices.
...
* I dropped all the variable (columns) aside from date prices and zip code
* Set zip code to the index
...
### All of the zip codes look to have ACFs that are significant up to around 25 and PACFs up to 2.
...
# Initial Models
### I decided to use an autoregressive model (ar) with 1 term as my baseline. I then compared it to a moving average model (ma) with 1 term, an ARMA model, and an ARIMA model (an ARMA model differenced once). Additionally, while all the prior models only used one term I used grid searching to automatically find the best terms for another ARIMA model and SARIMA model.
### I compared these models by their AIC scores and RMSE's. Instead of using traditional RMSE numbers I converted them to percentages, by dividing the RMSE by the average price of homes in each zip code, to more easily compare. 
...
### In the AIC and test RMSE results the autoSARIMA did best, with the exception of 10128 in test RMSE. Because of this I selected the autoSARIMA model for my final price predictions.
...
### Above you can see results of each model's prediction. All but one show positive values. And they all appear to have predicted values that are fairly close to their actual values.

# Conclusion
### My final models predict 47281 (Vallonia, Indiana) and 73064 (Mustang, Oklahoma) to have the best returns around 4%. Given the national average 18 month return of 5.2% this is a bit low but given my focus on lower risk investments this could make sense for more risk averse investors. Additionally, 10128 (New York City, New York) is expected to show a loss in this period so investors should avoid investing there. On a final note, risk averse investors looking to invest across multiple zip codes might consider investing in 47220 (Brownstown, Indiana) given it's lower but still positive return.

# Future work
### Given my focus on lower risk investing it is worth analyzing how these zip codes did during of economic down turns to confirm that they are low risk. I would also like to look at other zip codes that did well during the Great Recession to see if they potentially offer better returns, lower risk, or simply to diversify. It is also worth looking into how using leverage/debt might affect expected returns for the less risk averse. And finally, adding exogenous variables could help imporove my model's fit and make better predictions.
